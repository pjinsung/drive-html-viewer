...
> `--load-extension`은 Chrome이 실행 중이면 작동하지 않습니다. Chrome이 재시작됩니다.
...
"C:\Program Files\Google\Chrome\Application\chrome.exe" --profile-directory=Default --load-extension="$(pwd)/drive-html-viewer" --restore-last-session

# macOS
pkill -f "Google Chrome"
"/Applications/Google Chrome.app/Contents/MacOS/Google Chrome" --profile-directory=Default --load-extension="$(pwd)/drive-html-viewer" --restore-last-session

# Linux
pkill chrome
google-chrome --profile-directory=Default --load-extension="$(pwd)/drive-html-viewer" --restore-last-session
```

`--restore-last-session` 플래그가 재시작 전 열려있던 탭을 복원합니다.
